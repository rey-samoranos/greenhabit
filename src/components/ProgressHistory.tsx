...
const ProgressHistory: React.FC<ProgressHistoryProps> = ({ onBack, onLogout }) => {
  ...
  const saveEditedProgress = () => {
    if (selectedPlan) {
      ...
      // Update in savedPlans array
      const updatedPlans = savedPlans.map(plan => 
        plan.id === selectedPlan.id ? updatedPlan : plan
      );
      
      localStorage.setItem('greenhabit_plans', JSON.stringify(updatedPlans));
  ...
  // NEW: Clear History Confirmation Modal Component
  ...
  // NEW: Clear all history - opens confirmation modal instead of window.confirm
  const handleClearHistoryClick = () => {
    setShowClearConfirmModal(true);
  };

  // NEW: Confirm clear history
  const confirmClearHistory = () => {
    localStorage.removeItem('greenhabit_plans');
    setSavedPlans([]);
    setShowClearConfirmModal(false);
  };

  // NEW: Cancel clear history
                ...
                {[...savedPlans].reverse().map((plan) => {
                  ...
                  return (
                    <div key={plan.id} className="relative flex gap-4">
                      ...
                      <div 
                        className="flex-1 bg-green-900/30 backdrop-blur-sm rounded-2xl p-6 border border-green-500/30 hover:bg-green-800/30 transition cursor-pointer"
                      ...
                      >
                        ...
                        <p className="text-green-200 text-sm font-fredoka">
                          {calculateImpact(plan)}
                        </p>
                        
                        <button className="mt-3 text-[#81C784] text-sm hover:text-white transition font-fredoka">
                          View Details →
                        </button>
                      </div>
                    </div>
