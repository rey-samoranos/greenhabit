...
const HabitPlanner: React.FC<HabitPlannerProps> = ({ 
  ...
  const saveCurrentProgress = () => {
    if (habits) {
      ...
      const existingIndex = plans.findIndex((p: any) => 
        p.goal === goal && p.startDate === savedPlan.startDate
      );
      ...
      localStorage.setItem('greenhabit_plans', JSON.stringify(plans));
      setHasSaved(true);
  ...
  const saveCompletedPlan = () => {
    if (completedCount === 7 && !hasSaved && habits) {
      saveCurrentProgress();
    }
  };

  const generateHabitPlan = () => {
    setLoading(true);
    
    setTimeout(() => {
      const habitPlans: Record<string, string[]> = {
        plastic: [
          "Say 'no' to plastic straws - bring your own reusable one",
          "Bring reusable shopping bags instead of taking plastic bags",
          "Use a refillable water bottle instead of buying bottled water",
          "Choose products with glass or paper packaging over plastic",
          "Refuse plastic utensils - carry your own bamboo set",
          "Buy in bulk to reduce plastic packaging waste",
          "Properly recycle all plastic containers after rinsing"
        ],
        water: [
          "Take shorter showers - aim for 5 minutes max",
          "Turn off the tap while brushing your teeth",
          "Only run washing machine with full loads",
          "Fix any leaky faucets or running toilets",
          "Collect rainwater for watering plants",
          "Use a water-efficient dishwasher and run only when full",
        ...
        energy: [
          ...
          "Use natural light during the day instead of artificial lights"
        ],
        carbon: [
          "Walk or bike for trips under 1 mile instead of driving",
          "Take public transit or carpool to work once this week",
          "Eat plant-based for one full day",
          "Buy one item from a local thrift store instead of new",
          "Compost your food scraps instead of sending to landfill",
          "Work from home if possible to reduce commute emissions",
          "Plant or support planting of one tree"
        ]
      };

      setHabits(habitPlans[goal] || habitPlans.plastic);
      setLoading(false);
    }, 2000);
  };
        ...
        <div className={`bg-gradient-to-br ${getGoalColor()} rounded-2xl p-8 shadow-2xl border border-white/20`}>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
            <button
              ...
              className="px-6 py-3 bg-white/20 backdrop-blur-sm text-white font-semibold rounded-xl hover:bg-white/30 transition shadow-md font-spartan border border-white/30 flex items-center gap-2 justify-center"
            >
